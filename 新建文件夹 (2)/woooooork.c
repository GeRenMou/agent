...
void init_books() {
    ...
    // 第二本书
    books[1].id = 1002;
    strcpy(books[1].name, "长跑技巧");
    strcpy(books[1].author, "霍金");
    books[1].count = 3;

    // 第三本书
    books[2].id = 1003;
    strcpy(books[2].name, "直升机组成原理");
    strcpy(books[2].author, "科比.布莱恩特");
    books[2].count = 4;

    // 第四本书
    books[3].id = 1004;
    strcpy(books[3].name, "微软大战代码");
    strcpy(books[3].author, "比尔.盖茨");
    books[3].count = 6;

    // 第五本书
    books[4].id = 1005;
    strcpy(books[4].name, "abcdefg");
    strcpy(books[4].author, "我");
    books[4].count = 2;

    // 第六本书
    books[5].id = 1006;
    strcpy(books[5].name, "bcdefgh");
    strcpy(books[5].author, "不是我");
    books[5].count = 7;
    ...
    // 第八本书
    books[7].id = 1008;
    strcpy(books[7].name, "和溢位");
    strcpy(books[7].author, "clau");
    books[7].count = 8;

    // 第九本书
    books[8].id = 1009;
    strcpy(books[8].name, "愿神");
    strcpy(books[8].author, "米哈优");
    books[8].count = 2;

    // 第十本书
    books[9].id = 1010;
    strcpy(books[9].name, "骂谁罕见");
    strcpy(books[9].author, "东洋雪莲");
    books[9].count = 3;
...
void borrow_book() {
    ...
    int i = 0;
    while (input[i] != '\0') {
        borrow_id = borrow_id * 10 + (input[i] - '0');
        i++;
    }

    // 检查编号范围
    if (!check_id_range(borrow_id)) {
    ...
    // 查找图书
    int book_index = check_id_exists(borrow_id);
    ...
    printf("当前库存: %d\n", books[book_index].count);

    // 检查库存
    if (books[book_index].count <= 0) {
        printf("\n很抱歉，该书库存为0，无法借阅！\n");
        printf("\n按任意键返回主菜单...");
        getchar();
        getchar();
...
void add_book() {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (is_number(input)) {
            ...
            // 检查编号是否已存在
            else if (check_id_exists(new_id) != -1) {
                printf("错误：编号 %d 已存在！\n", new_id);
                id_valid = 0;
            }
            else {
